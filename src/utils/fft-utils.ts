...
export function ifftFieldElements(
  ...
  if (logN % 1 !== 0) {
    throw new Error(`IFFT requires power of 2, got ${n} (domainSize=${domainSize}, values.length=${values.length})`)
  }
  
  if (n !== domainSize) {
    throw new Error(`Padded values length ${n} must equal domain size ${domainSize}`)
  }
