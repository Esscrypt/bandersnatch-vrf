...
import { prove_ring_proof as wasmProveRingProof } from '../../wasm-ark-vrf/ark_vrf_wasm'
...
function replaceNullKeyWithPadding(keyBytes: Uint8Array): Uint8Array {
  if (keyBytes.every((b) => b === 0)) {
    return hexToBytes(BANDERSNATCH_VRF_CONFIG.PADDING_POINT as Hex)
  }
  return keyBytes
}
...
describe('Ring VRF End-to-End Tests (WASM)', () => {
  ...
  let srsBytes: Uint8Array
  let srsFilePath: string

  beforeAll(async () => {
    srsFilePath = path.join(__dirname, 'test-data/srs/zcash-srs-2-11-uncompressed.bin')
    srsBytes = readFileSync(srsFilePath)
    ...
    for (const [index, vector] of RING_TEST_VECTORS.slice(0, 2).entries()) {
      test(`Vector ${index + 1}: Exact value comparison with test vector`, async () => {
        ...
        try {
          ...
          // ===== Ring Proof (byte-for-byte match with reference) =====
          // Compare native proof to ark-vrf-wasm with the same inputs (reference).
          if (proofResult.proof.ringProof) {
            const actualRingProof = bytesToHex(proofResult.proof.ringProof).slice(2)
            const ringKeysBytes = new Uint8Array(ringInput.ringKeys.length * 32)
            for (let i = 0; i < ringInput.ringKeys.length; i++) {
              ringKeysBytes.set(replaceNullKeyWithPadding(ringInput.ringKeys[i]), i * 32)
            }
            const pedersenForBlinding = PedersenVRFProver.prove(secretKey, {
              input: ringInput.input,
              auxData: ringInput.auxData,
            })
            const ringSize = ringInput.ringKeys.length
            const wasmProofBytes = wasmProveRingProof(
              srsBytes,
              ringKeysBytes,
              pedersenForBlinding.blindingFactor,
              ringInput.proverIndex,
              ringSize,
            )
            const expectedRingProofHex = bytesToHex(wasmProofBytes).slice(2)

            let firstDiffIndex = -1
            const minLen = Math.min(actualRingProof.length, expectedRingProofHex.length)
            for (let i = 0; i < minLen; i += 2) {
              if (actualRingProof.slice(i, i + 2) !== expectedRingProofHex.slice(i, i + 2)) {
                firstDiffIndex = i / 2
                break
              }
            }
            if (firstDiffIndex === -1 && actualRingProof.length !== expectedRingProofHex.length) firstDiffIndex = minLen / 2

            console.log(`\n--- Ring Proof (native vs ark-vrf-wasm reference) ---`)
            console.log(`Native length: ${proofResult.proof.ringProof.length} bytes`)
            console.log(`WASM length:   ${wasmProofBytes.length} bytes`)
            if (actualRingProof === expectedRingProofHex) {
              console.log(`✅ Native ring proof matches ark-vrf-wasm (byte-for-byte)`)
            } else {
              console.log(`Expected (WASM first 64): ${expectedRingProofHex.slice(0, 64)}...`)
              console.log(`Actual (native first 64): ${actualRingProof.slice(0, 64)}...`)
              if (firstDiffIndex >= 0) console.log(`First diff at byte index: ${firstDiffIndex}`)
            }

            expect(actualRingProof).toBe(expectedRingProofHex)
          }
